...
commons = {
    ...
    "gpu_h100": "Use the gpu partition for jobs that make use of GPUs. You must [request GPUs explicitly](/clusters-at-yale/job-scheduling/resource-requests/#request-gpus) with the `--gpus` option in order to use them. For example, `--gpus=h100:2` would request 2 NVIDIA H100 GPUs per node.",
    ...
    "gpu_b200": "Use the gpu partition for jobs that make use of GPUs. You must [request GPUs explicitly](/clusters-at-yale/job-scheduling/resource-requests/#request-gpus) with the `--gpus` option in order to use them. For example, `--gpus=b200:2` would request 2 NVIDIA B200 GPUs per node.",
...
def get_part_hardware():
    ...
    for part_line in check_output(
        ...
        if line_dict["partition"] != "":
            ...
            cpu_type = [cpu_regex.match(x) for x in line_dict["features"].split(',') if cpu_regex.match(x)][0][0]
            gpu_type = []
            gpu_mem = []
            gpu_num = []
...
def sort_hardware(partition_hardware):
    ...
    for node_type in partition_hardware:
        for feature in node_type["Node Features"].split(", "):
            if feature.split(':')[0] == 'cpugen':
                node_gen = feature.split(':')[1]
                break
