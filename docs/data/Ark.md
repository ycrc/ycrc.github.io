...
- **Windows**: Mount `\\ark.ycrc.yale.edu\<sharename>` using `yale\netid` as the username and your NetID password.
- **Mac**: In Finder, use "Go → Connect to Server" and enter `smb://ark.ycrc.yale.edu/<sharename>` as a "registered user," authenticating with your NetID and password.
- **Linux** (including on YCRC clusters): Use `smbclient` on the command line:
  ```
  smbclient //ark.ycrc.yale.edu/sharename -U netID -W Yale
  ```
