...
!!! warning "Use with Caution"
    The `qdirstat` and `broot` programs are intended to be used only very infrequently or for small collections of files, because they can put a heavy load on the cluster storage systems when there are many files to be tallied. If you are dealing with a very large collection of files, please contact us at research.computing@yale.edu. We have specialized tools that minimize the filesystem load for these types of problems.
