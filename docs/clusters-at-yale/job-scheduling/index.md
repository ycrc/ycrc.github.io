...
|Long Option<img width=120/>|Short Option|Default            |Description|
|---------------------------|------------|-------------------|-----------|
...
Being a good cluster citizen means not over-allocating resources to a job. However, under-allocating resources can severly impact its performance and success. Slurm strictly enforces limits on the CPUs, GPUs, and memory you request. If you try to use more memory than your job was allocated, your applications will probably [die with strange-sounding error codes](/clusters-at-yale/job-scheduling/common_job_failures/#running-out-of-memory). If you request a single CPU and start an application that runs many parallel threads, those threads will be forced to share one CPU and run very slowly (probably slower than a single-threaded version of the same program!).
...
You can submit a script as a batch job, *i.e.* one that can be run non-interactively in batches. These scripts are comprised of three parts:
...
When the job finishes the output should be stored in a file called `slurm-jobid.out`, where `jobid` is the submitted job's ID.
